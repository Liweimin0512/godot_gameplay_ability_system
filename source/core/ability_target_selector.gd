...
## 获取可用目标
func get_available_targets(ability: Ability, context: AbilityContext) -> Array:
	return []


## 验证目标
func validate_targets(ability: Ability, targets: Array, context: AbilityContext) -> bool:
	return false
...
func get_actual_targets(ability: Ability, context: AbilityContext) -> Array:
	return context.get_all_targets()


## 应用目标过滤器
func apply_filters(targets: Array, filters: Array, context: AbilityContext) -> Array:
...
## 应用单个过滤器
func _apply_filter(targets: Array, filter: Dictionary, context: AbilityContext) -> Array:
