...
## 验证目标
func validate_targets(ability: Ability, targets: Array, context: Dictionary) -> bool:
	return false
...
## 应用目标过滤器
func apply_filters(targets: Array, filters: Array, context: Dictionary) -> Array:
...
## 应用单个过滤器
func _apply_filter(targets: Array, filter: Dictionary, context: Dictionary) -> Array:
	...
	if filter.is_empty() or filter_type == "":
		...
		return targets.filter(func(t): 
			return t.global_position.distance_to(context.owner.global_position) <= filter.max_distance
		)
	return targets
