...
func _check_costs(context: Dictionary) -> bool:
	...
	if not caster:
		...
		return false
	_cost_value = _resolve_parameter(cost_value_param, AbilityContext.from_dictionary(context))
	var ability_resource_component : AbilityResourceComponent = caster.ability_resource_component

	if not ability_resource_component.has_enough_resources(cost_resource_id, _cost_value):
		return false
	return true
...
func _apply_costs(context: Dictionary) -> void:
	...
	if not caster:
		...
		return
	var ability_resource_component : AbilityResourceComponent = caster.ability_resource_component
	ability_resource_component.consume_resources(cost_resource_id, _cost_value)
